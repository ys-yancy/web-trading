...
module.exports = {
  ...
  getTime: function(timestr) {
    return Focha.parse(timestr, 'YYYY-MM-DD HH:mm:ss').getTime();
  },
  ...
  isDaily: function() {
    return location.host.indexOf('localhost') !== -1 || location.host.indexOf('test.') !== -1;
  },
  ...
  getMinQuoteUnit(minQuoteUnit) {
    try {
      minQuoteUnit = minQuoteUnit.split('.')[1].split('').length;
    } catch (e) {
      minQuoteUnit = minQuoteUnit;
    }
