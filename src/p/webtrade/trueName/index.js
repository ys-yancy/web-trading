...
class Truename extends Base {
    constructor() {
        ...
        this.login().then(() => {
            new Header();
            this._bind();
            $('#truephone').val(this.cookie.get('phone'));
            this.onlyOne = false;
            ...
            if(getTrueMsg()){
              ...
              $('#agreeTrueMsg').css('display','block');
            }
        });
    }
    ...
    _getParams() {
        ...
        return {
          ...
          bank_name:$('#mybanks').find('option:selected').text(),
          bank_code:$('#mybanks').val(),
          card_no: $('.J_BankId').val(),
    ...
    _validates() {
      var self = this;
      var els = ['.J_CardName', '.J_CardId', '.J_BankId'];
      var types = ['empty', 'cardId', 'bankId'];
      var pass = true;
